...
class YOLOXHeadKPTS(nn.Module):
    def __init__(
        ...
        self.n_bimoetry_features = 3 # Currenntly there is no biometry features but in future it can be emotion, age, etc can be estimated from face features
        self.kpt_index = []
        ...
        for i in range(len(in_channels)):
            ...
            self.biometry_preds.append(
                nn.Conv2d(
                    in_channels=int(256 * width),
                    out_channels=self.n_anchors * self.n_bimoetry_features, 
                    kernel_size=1,
                    stride=1,
                    padding=0,
                )
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        for k, (cls_conv, reg_conv, kpts_conv, biometry_conv, stride_this_level, x) in enumerate(
            ...
            kpts_output = self.kpts_preds[k](kpts_feat) # e.g Bx51x80x80. 5 key points
            
            biometry_feat = biometry_conv(biometry_x)
            biometry_output = self.biometry_preds[k](biometry_feat) # e.g Bx3x80x80. 3 biometry features. Current dont has but in future it can be emotion, age, etc
            ...
            if self.training:
                ...
                if self.use_l1:
                    ...
                    biometry_output = biometry_output.view(
                        batch_size, self.n_anchors, self.n_bimoetry_features, hsize, wsize
                    )
                    biometry_output = biometry_output.permute(0, 1, 3, 4, 2).reshape(
                        batch_size, -1, self.n_bimoetry_features
                    )
                ...
                # devunote: The indice is 4:6 because the output is [x, y, w, h, obj, cls, kpts,biometry_output]. And cls here seems to be 1-size because there is only human class has keypoints
    ...
    def get_losses(
        ...
        s_index_of_biometry = 5+self.num_classes+3*self.num_kpts
        e_index_of_biometry = s_index_of_biometry+self.n_bimoetry_features
        biometry_preds = outputs[:, :, s_index_of_biometry:e_index_of_biometry]  # [batch, n_anchors_all, 3]
        ...
        loss_biometry = (
            self.l1_loss(biometry_preds.view(-1, self.n_bimoetry_features)[fg_masks], biometry_targets)
        ).sum() / num_fg
        ...
        # Add loss for biometry in future
        # loss = loss + biometry_weight * loss_biometry 
