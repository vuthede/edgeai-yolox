...
def preprocx(img, input_size, swap=(2, 0, 1)):
    resized_img = cv2.resize(
        img,
...
class ValTransform:
    ...
    def __call__(self, img, res, input_size):
        ...
        return img, np.zeros((1, 5)), np.zeros((1, 24))
...
class ValTransformX:
    ...
    # assume input is cv2 img for now
    def __call__(self, img, res, input_size):
        img, _ = preprocx(img, input_size, self.swap)
        if self.legacy:
            img = img[::-1, :, :].copy()
            img /= 255.0
            img -= np.array([0.485, 0.456, 0.406]).reshape(3, 1, 1)
